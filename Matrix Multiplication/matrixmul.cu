...
int main(int argc, char** argv) {
	...
	if(argc != 5 && argc != 4) 
	{
		...
		// Checking until elements are less than 64000
		while(1)
		{
			a = rand() % 1024;
			b = rand() % 1024;
...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
    ...
    dim3 blockPerGrid(ceil(N.width/(float)BLOCK_SIZE), ceil(M.height/(float)BLOCK_SIZE), 1);
 
    // Launch the device computation threads!

    MatrixMulKernel<<< blockPerGrid, threadPerBlock>>>(Md, Nd, Pd);
